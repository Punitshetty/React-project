...
function App() {
    ...
    const sortOptions = [ "date_of_birth", "salary"];
  ...
  const handleSort = async (e) => {
    ...
    if (value === "date_of_birth") {
      ...
      const filteredData = data.filter((item) => item.salary);
      const sortedData = filteredData.sort((a, b) => {
        const dateA = new Date(a.salary);
        const dateB = new Date(b.salary);
        return dateB - dateA ; 
      });
      setData(sortedData);
    }else {
      try {
        const response = await axios.get('http://localhost:3001/employees?_sort=${value}&_order=asc');
        setData(response.data);
      } catch (err) {
        console.log(err);
      }
